  ...
  methods: {
    ...
    getTitles (textes, target, elems) {
      ...
      textes.forEach(text => {
        ...
        let headingEnd = this.getHeadingsIndex(text, target[0][1])

        let subHeadingStart = this.getHeadingsIndex(text, target[1][0])

        let subHeadingEnd = this.getHeadingsIndex(text, target[1][1])
        let headingsIndexes = this.concatHeadingIndexes(
          headingStart,
          headingEnd
        )

        let subHeadingsIndexes = this.concatHeadingIndexes(
          subHeadingStart,
          subHeadingEnd
        )
        let headingCode = this.sliceText(text, headingsIndexes)

        let subHeadingsCode = this.sliceText(text, subHeadingsIndexes)
